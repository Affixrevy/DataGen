...
MAX_VAL = 26
def station_125():
    f = open("OUT_____+"+str(START_BIKES)+"extra.txt","w+")
    f.write("carbon:\n")
...
def move_to_first_can_be_colletted(v_plus,tsp,truck,carbon,sum_distance):
    if Truck.full_load-truck.get_num_bikes()>=(v_plus.get_num_bikes()) or pos_to_id(v_plus.get_pos(),tsp) == 0:
        ...
        if v_plus.id >= 90:
            pass
            #print("v_plus: ",v_plus.id,"truck_position: ", pos_to_id(truck.get_pos(),tsp))
...
def generate_carbon(stations):
    ...
    for i in range(len(tsp)):
        sum_bikes+= tsp[i].get_num_bikes()
        #print(tsp[i].id, tsp[i].get_num_bikes())
        if i != len(tsp)-1:
            sum_dis_tsp+=getDistance(tsp[i].get_pos(),tsp[i+1].get_pos())

    print("tsp_dist: ", sum_dis_tsp)
    tsp[-1].num_b = 0
    if sum_bikes >= 0:
        truck = Truck(0+START_BIKES,depot_x,depot_y)
    else:
        truck = Truck(abs(sum_bikes)+START_BIKES,depot_x,depot_y)
    ...
    while edge_index<=len(tsp)-1 and p<=120 and truck_pos != 0:
        p+=1
        ...
        #print(sum_distance)
        if truck.get_num_bikes()>=abs(v_minus.get_num_bikes()) and truck.get_num_bikes() != 0 and v_minus.id != 0:
        ...
        elif get_index(tsp,v_minus)<get_index(tsp,v_plus) and truck.get_num_bikes()+v_plus.get_num_bikes()>abs(v_minus.get_num_bikes()) and v_minus.id != 0:
            ...
            #print("3_step_v_plus: ",v_plus.id,"edge_index: ",edge_index)
            truck.move_to(v_plus.get_pos())
            truck.change_bikes(v_plus.get_num_bikes(), v_plus)    
    ...
    #print(truck_pos,"truck bikes:", truck.get_num_bikes())
    #print("v_minus id: ",v_minus.id,"v_plus id: ", v_plus.id)
    '''
    for i in range(len(tsp)):
        sum_bikes+= tsp[i].get_num_bikes()
        print(tsp[i].id, tsp[i].get_num_bikes())
